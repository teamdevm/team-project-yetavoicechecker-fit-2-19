...
def skhozh (x,y):
    ...
    return (a)
#добавление нового голоса или уыеличение количества раз у старого ниже двух
...
for results in result:
    ...
    if (fa>0.85):
      ...
      for xy in results[0]:
          spktr=spktr+str(xy)+','
      spktr=spktr[:len(spktr)-1]+"}'"
      db_object.execute(f"UPDATE moshenniki SET times=times+1 WHERE spektr={spktr}::double precision[]")
      db_connection.commit()
      b=1
if (b==0):
    db_object.execute("INSERT INTO moshenniki(spektr,times) VALUES (%s,%s)", (x,0))
    db_connection.commit()
